...
string[] thirdsimvol(string[] stringArray) //Основная функция
{
    ...
    for (int i = 0; i < stringArray.Length; i++)
    {
        if (stringArray[i].Length <= 3)
        {
            rezult[j] = stringArray[i]; //Внос элементов с тремя и менее символами с введенного массива в новый
            j++;
        }
    }
    return rezult;
...
void printArray(string[] stringArray) //Функция вывода массива
{
    Console.Write("[");
    for (int i = 0; i < stringArray.Length; i++)
    {
